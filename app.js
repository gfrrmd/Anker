...
const TIBA_VISIBLE_SECONDS = 3; // tampilkan "Tiba" selama 3 detik
...
function cdStr(arrMin){
  ...
  // 0 s/d -3 detik → tetap tampil "Tiba"
  if (totalSec <= 0 && totalSec >= -TIBA_VISIBLE_SECONDS) {
    return 'Tiba';
  }
...
function getUpcoming(limit=20){
  ...
  for(const t of SCHEDULE){
    ...
    // sudah lewat lebih dari 3 detik → buang
...
function render(){
  ...
  // label kecil "ke arah ..."
  document.querySelector('.hero-cd-lbl').textContent = `ke arah ${f.dest}`;
...
function selectStation(name){
  ...
  closeSearch();   // tutup overlay & keyboard dulu
  buildDirTabs();
  render();
}
...
function tickSecond(){
  ...
  document.querySelectorAll('[data-arrival]').forEach(el => {
    ...
    }else if(el.classList.contains('ti-cd')){
      el.className = cls ? `ti-cd ${cls}` : 'ti-cd';
    }
  });

  // kalau kereta sudah lewat >3 detik, list di-render ulang
  if(curStation){
    const before = document.querySelectorAll('.train-item').length;
    const newList = getUpcoming();
    if (!newList.length || newList.length + 1 !== before) {
      render();
    }
  }
}
...
// restore stasiun terakhir kalau ada
